...
根拠は PR #21 で実際に起きたことである。開発依存3件がまとめて提案され、**CI は緑だった**が、
**2件は受け入れてはいけないものだった。**
...
**代償。** 依存の版は自動では上がらない。上げるのは必要が生じたときの手作業になる。
脆弱性を直す更新も自動では PR にならない。
**ただし検知は [audit.yml](.github/workflows/audit.yml) が毎週続ける。**
落ちたら、そのとき直す版を人が選ぶ。**「気づけない」状態にはならない。**
