...
| 環境変数 | 既定 | 意味 |
|---|---|---|
| `TRUST_PROXY_HOPS` | 0 | 信頼する中継の段数（Express の `trust proxy`）。**レート制限の発信元はこれで決まる。** 手元で直接叩くなら 0。本番（CloudFront → ALB）は 2 で、**ALB に CloudFront を経ずに届く経路を塞いでいることが前提**（塞いでいないと X-Forwarded-For で発信元を偽れる）。多すぎると偽の発信元を名乗れ、少なすぎると全員が1つの発信元として数えられる |
