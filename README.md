...
**`POSTGRES_PASSWORD` は英数字だけにする。** 下の接続 URL に組み込むため、
`@` `/` `?` `#` `%` `:` は区切りとして解釈される。**特に `@` は別の宛先に繋ごうとする**形になり、
...
**`.env` を作り直すときは、先に `docker compose down -v` を済ませる。**
