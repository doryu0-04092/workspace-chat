...
| [検証の記録](docs/verification.md) | 2026-09-24 にステージングと本番を AWS に立てて行った検証の結果・未解決・確かめていないこと |
...
| [audit.yml](.github/workflows/audit.yml) | 依存の脆弱性検査と、**秘密の値がソースに書かれていないかの検査**（gitleaks）。PR・push に加えて**毎週月曜に定期実行する**（走査範囲がイベントで変わることの実測は同ファイルのコメントに記す）（要件定義書 4.3 の「継続的に」） |
...
**手元でテストを回さないなら、これを叩かない限り、最初に `up` した日の 8.x のまま動き続ける**
（テストは同じタグを使い、`npm test` のたびに取り直す。下の一覧）。
...
- **redis（`valkey/valkey:8-alpine`）: `docker compose pull redis` のほかに、`npm test` のたびに取り直す経路がある**
  ...
  **どちらも取り直すのは手元のイメージまでで、動いているコンテナに入るのは次に `up -d` で作り直したときである。**
...
- 作り直した環境は **URL が変わり、DB は空から始まる**。利用者・メッセージは引き継がない
