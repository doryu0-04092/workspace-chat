...
**api の起動には環境変数 `WEB_ORIGIN`（web の origin）も要る。** リフレッシュとログアウトの要求の `Origin` / `Referer` と突き合わせる（CSRF の対処。要件定義書 4.3）。**スキーム://ホスト[:ポート] の形で、末尾の `/` を付けない**（付けると起動時に落ちる）。手元では web の開発サーバーの origin（例: `http://localhost:5173`）を渡す。
