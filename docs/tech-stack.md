...
| 項目 | 採用 | 備考 |
|---|---|---|
| IaC | **Terraform** **1.11 以上** | 既存プロジェクトで実績あり。**1.11 以上とするのは、state のロックに S3 バックエンドの `use_lockfile` を使うためである**（HashiCorp の S3 バックエンドの文書: 1.9 の版には `use_lockfile` が無く、1.10 は「(Experimental, Optional)」、1.11 は「(Optional)」。下の「本番の HTTPS・秘密情報・state の置き場」）。**秘密の値を state に残さない write-only 引数も 1.11 以上を要する**（Terraform の文書「To use write-only arguments, you must use Terraform v.1.11 or later」）。**AWS プロバイダーは 5.88 以上とする**——使う write-only 引数が入った版である（プロバイダーのリリースノートの 5.87.0「resource/aws_ssm_parameter: Add `value_wo` write-only attribute」、5.88.0「resource/aws_db_instance: Add `password_wo` write-only attribute」） |
