...
| コンテナ | ECS Fargate（**ARM64**） | **CPU アーキテクチャは ARM64**（決定・2026-09-12・依頼側。#274）——下の「リソースのサイジング」の 0.25 vCPU / 0.5 GB × 2 タスクで、X86 より約 20% 安い（AWS Price List API〔AmazonECS / ap-northeast-1、effectiveDate 2026-07-01〕の Linux オンデマンド: vCPU 時間 X86 USD 0.05056・ARM USD 0.04045、GB 時間 X86 USD 0.00553・ARM USD 0.00442。#274 で引いた値）。**api のイメージのビルドの `--platform` と Dockerfile への記録は未反映**（#452）。**運用者から RDS へは ECS Exec で入る**（決定・同。#274。追加の費用なし）。**入る先は api のタスクではなく、マイグレーション用のイメージ（#271）を `aws ecs run-task` で動かしたタスクである**（決定・2026-09-14・作業側。依頼側の「一般的なエンジニア目線で判断できることは判断してほしい」という委任による。#452。そのタスク定義の `secrets` は `DATABASE_URL` だけ、タスクロールは api のタスクと分けて ECS Exec に要る権限だけ。psql はマイグレーション用のイメージに入れる——未反映。#457。**入る手順（`--enable-execute-command`・command の上書き・終わったら止める）、運用者が ECS Exec で入る経路が届く範囲、入る先を api のタスクにしない理由は [要件定義書](requirements.md) 4.2「RDS の障害から復旧するとき」手順 5 の代償**）。**マイグレーションは `aws ecs run-task` で、マイグレーション用のイメージ（#271）を1回動かし、新しいタスク定義に切り替える前に流す**（決定・同。#274） |
