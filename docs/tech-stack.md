...
添付・アバターの本番は S3 だが、**ローカルとテストでは S3 互換ストレージの MinIO（`quay.io/minio/minio`）で代える**（#427。`compose.yaml`）。上の「添付ファイル」の行にある、署名者と確定の主体を分ける本番の設計は、**ローカルでは分けない**——api が単一の管理者の資格情報で読み書きする（分離の確認は本番で行う）。
