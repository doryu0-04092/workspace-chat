...
| 在席判定（F-22 / F-21） | 接続イベントから取得できる（**取れるのは自タスクに繋がっている分だけである。タスク間の伝達は [要件定義書](requirements.md) 4.2 の決定による**） |
