...
| Terraform の state | **S3 バケット（バージョニングあり）＋ `use_lockfile`**。バケットは本体の構成の外で先に作る（`dynamodb_table` は非推奨）。**構成の置き場は `infra/bootstrap`（state のバケット。この構成の state は手元のファイルに置く）と `infra/production`（本体。バケット名は `init` の `-backend-config` で渡す）**（#452）。**代償: `infra/bootstrap` の state を失うと、バケットは残る（`prevent_destroy`）が Terraform からは操作できず、取り込み直し（`terraform import`）が要る** |
