...
| コンテナ | ECS Fargate（**ARM64**） | **CPU アーキテクチャは ARM64**（決定・2026-09-12・依頼側。#274）——下の「リソースのサイジング」の 0.25 vCPU / 0.5 GB × 2 タスクで、X86 より約 20% 安い（AWS Price List API〔AmazonECS / ap-northeast-1、effectiveDate 2026-07-01〕の Linux オンデマンド: vCPU 時間 X86 USD 0.05056・ARM USD 0.04045、GB 時間 X86 USD 0.00553・ARM USD 0.00442。#274 で引いた値）。**api のイメージのビルドの `--platform` と Dockerfile への記録は未反映**（#452）。**運用者から RDS へは ECS Exec で api のタスクに入る**（決定・同。#274。追加の費用なし。#274 はイメージに psql が要るとしており、未反映。#457）。**マイグレーションは `aws ecs run-task` で、マイグレーション用のイメージ（#271）を1回動かし、新しいタスク定義に切り替える前に流す**（決定・同。#274） |
