...
- **write-only 引数の値は state に保存されないため、`plan` は値の変更を差分として出さない。** 値を替えるときは `value_wo_version`・`password_wo_version` を上げる（Terraform の文書「Terraform does not store write-only arguments in state files, so Terraform has no way of knowing if a write-only argument value has changed.」）
...
- **RDS をスナップショットから作り直す `apply` で、`password_wo` がマスターパスワードを入れ替えるか**（入れ替わるなら、`DATABASE_URL` のパラメータと食い違う。[要件定義書](requirements.md) 4.2 の RDS の復旧手順 3 の前に確かめる）
