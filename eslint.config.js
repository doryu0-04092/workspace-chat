...
export default tseslint.config(
  ...
  // 使うルールをこの2つに絞って明示する。
  //
  // **踏むと壊れる: 下の rules の2行は、scripts/lint-scope.test.sh が
  // 文字列の完全一致で参照している。** 壊す確認は exhaustive-deps の重大度を
  // 書いた行が**ちょうど1件**あることを求め、それを warn に置き換えて走らせる。
  // そのため **重大度をオプション付きの配列（`[..., { ... }]`）の形に変えると、
  // npm run lint は通るのにあの検査だけが「sed が想定どおりに書き換えられて
  // いない」で落ちる。** 同じ理由で、**この注記に重大度付きの行をそのまま
  // 書き写してもならない**（件数が2件になって同じ落ち方をする。実測）。
