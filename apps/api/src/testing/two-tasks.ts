...
/**
 * 別のタスクへアダプタ（Valkey）を通って届くもの（サーバー間の通知・部屋への配信・部屋から外す処理）を待つ上限。
 * **踏むと壊れる: 手元の速さで決めない。** CI（テストのファイルを並べて流し、コンテナも並ぶ）では、手元なら数百ミリ秒で届くものが
 * 3 秒を超えて落ちた（2026-09-17・18 の main の CI。在席・部屋・@here の検査が、変更と無関係なコミットで約 3.2〜3.4 秒で落ちた）。
 * 届けばすぐに次へ進むため、上限を長くしても通るときの所要時間は変わらない。
 */
export const CROSS_TASK_WAIT_MS = 15_000;

/**
 * 「届かないこと」を確かめるときに待つ時間。**届くはずの遅れた配信が、この後に紛れ込まない長さにする**（CI では 0.7 秒では足りなかった）。
 */
export const CROSS_TASK_QUIET_MS = 2_000;

/** 別のタスクを待つ検査の、1件あたりの上限（既定の 5 秒では、上の待ちを1回使い切るだけで落ちる）。 */
export const CROSS_TASK_TEST_TIMEOUT_MS = 60_000;
