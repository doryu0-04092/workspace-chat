 ...
 *
 * **踏むと壊れる: 既定の 3000 は、本番の構成も infra/production/compute.tf の locals の api_port で持つ**（ターゲットグループと
 * infra/production/network.tf の ALB→タスクの規則はその値を参照する）。既定を変えるならそちらも直す。
 * 食い違うと ALB からタスクへ届かず、CI は緑のまま、デプロイ後に初めて落ちる。
 */
