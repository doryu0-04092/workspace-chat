...
export type RealtimeValkeyClients = { readonly publisher: Redis; readonly subscriber: Redis };
...
export function createRealtimeValkeyClients(base: Redis, logger: Logger): RealtimeValkeyClients {
  ...
  const publisher = withHandledRejections(base, (command, succeeded) => {
    ...
    if (succeeded && failing) logger.log('Valkey に戻ったため、タスクをまたいで配信する');
  ...
  return { publisher, subscriber: withHandledRejections(subscriber) };
}
