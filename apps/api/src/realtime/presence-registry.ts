 ...
 * - **warn は取り直しの失敗に切り替わったときに1回だけ出し、取り直せるようになったら log を出す**——入り直しの入室要求ごとに出すとあふれる
 *   （rate-limit/resilient-rate-limit-storage.ts・auth/login-backoff.ts と同じ形。#400）
 */
@Injectable()
export class PresenceRegistry implements OnApplicationBootstrap, OnModuleDestroy {
  ...
  /** 直近の取り直しが失敗したか（warn を切り替わったときに1回だけ出すため）。 */
  ...
  /** 取り直しの失敗に切り替わったときに1回だけ warn を出し、取り直せるようになったら log を出す。 */
  private reportRefresh(failures: readonly string[]): void {
    if (failures.length > 0) {
      ...
      this.failing = false;
      this.logger.log('在席の一覧を取り直せるようになった');
    }
  }
}
