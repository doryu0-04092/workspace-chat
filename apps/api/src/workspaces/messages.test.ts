...
/** 投稿・返信・編集・削除の上限（利用者単位で1分に60回。チャンネルと DM で1つの枠。機能一覧 4.1・4.2・6・8）。 */
const POST_LIMIT = 60;
...
describe('メッセージの投稿・一覧・編集・削除（F-11・F-12・F-13）', () => {
  ...
  describe('編集と削除', () => {
    ...
    // #384: 投稿・編集・削除は1つの枠を分け合う（提案・承認済・2026-09-13・依頼側）。返信も同じ枠に入れる（機能一覧 6 の実装時に決めた値）。DM の投稿・編集・削除も同じ枠に入る（機能一覧 8 の実装時に決めた値）。
