import { Body, Controller, HttpCode, HttpStatus, Param, Post, UseGuards } from '@nestjs/common';
import { Throttle } from '@nestjs/throttler';
import { type AuthenticatedUser, CurrentUser } from '../auth/access-token.guard';
import { UPLOAD_COMPLETE_LIMIT, UPLOAD_ISSUE_LIMIT } from '../file-uploads/upload-rate-limit';
import { UserRateLimitGuard } from '../rate-limit/user-rate-limit.guard';
import type { Attachment } from './attachment-view';
import type { UploadRequest, UploadTicket } from './attachments.service';
import { DmAttachmentsService } from './dm-attachments.service';

/**
 * DM の添付ファイルのアップロードの発行と確定（F-27・F-28・F-19。#239）。アクセストークンを求める（AccessTokenGuard の既定）。
 * 上限はチャンネルの添付と同じ値で、利用者単位（upload-rate-limit.ts）。入力の形は openapi-validation.ts が仕様で確かめる。
 */
@Controller('workspaces/:id/dms/:dmId/attachments/uploads')
export class DmAttachmentsController {
  constructor(private readonly attachments: DmAttachmentsService) {}

  @Post()
  @UseGuards(UserRateLimitGuard)
  @Throttle({ default: UPLOAD_ISSUE_LIMIT })
  issue(
    @CurrentUser() user: AuthenticatedUser,
    @Param('id') workspaceId: string,
    @Param('dmId') dmId: string,
    @Body() body: UploadRequest,
  ): Promise<UploadTicket> {
    return this.attachments.issue(user.id, workspaceId, dmId, body);
  }

  @Post(':uploadId/complete')
  @HttpCode(HttpStatus.OK)
  @UseGuards(UserRateLimitGuard)
  @Throttle({ default: UPLOAD_COMPLETE_LIMIT })
  complete(
    @CurrentUser() user: AuthenticatedUser,
    @Param('id') workspaceId: string,
    @Param('dmId') dmId: string,
    @Param('uploadId') uploadId: string,
  ): Promise<Attachment> {
    return this.attachments.complete(user.id, workspaceId, dmId, uploadId);
  }
}
