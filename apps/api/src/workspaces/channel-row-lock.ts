 ...
 *
 * - `share`（参加・招待・投稿・返信・編集・削除。channel-access.ts の `lockedChannelFor`）: アーカイブ・復元と同時に来ても、その確定を待ってから読み、アーカイブの後に人を増やさない・メッセージを書かない（機能一覧 3.2）。
 ...
 *   復元と同時に来た参加・招待・投稿・返信・編集・削除は確定前の値（アーカイブ済み）を読んで 409 を返す。`FOR SHARE` なら復元の確定を待ってから読む
 * - `update`（アーカイブ）: 読みと更新のあいだに採番と復元が確定して、番号を持つ行を採番し直すことを防ぐ（検査制約はこれを止めない）。
 *   `FOR SHARE` では、同時の2つのアーカイブが両方とも掴んだまま更新に進み、互いを待って行き詰まる
 * - `update`（ピン留めの付け外し。pins.service.ts）: 同じチャンネルへの同時のピン留めを直列にし、件数の上限を超えない。アーカイブ・復元の確定を待ってから読むのは `share` と同じ
 */
