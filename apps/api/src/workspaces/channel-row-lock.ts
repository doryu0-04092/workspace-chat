 ...
 *
 * - `share`（参加・招待・投稿・編集・削除。channel-access.ts の `lockedChannelFor`）: アーカイブ・復元と同時に来ても、その確定を待ってから読み、アーカイブの後に人を増やさない・メッセージを書かない（機能一覧 3.2）。
 ...
 *   復元と同時に来た参加・招待・投稿・編集・削除は確定前の値（アーカイブ済み）を読んで 409 を返す。`FOR SHARE` なら復元の確定を待ってから読む
