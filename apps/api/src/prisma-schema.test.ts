...
describe('Prisma のスキーマとマイグレーション', () => {
  ...
  describe('一意制約', () => {
    ...
    it('チャンネル名の事実上の上限は、文字数では決まらない', async () => {
      // **チャンネル名の数値の上限（50 コードポイント。#290）は入力の仕様に置き、列には置かない**（機能一覧 3.1）。
  ...
  describe('プライベートチャンネルの可視性', () => {
     ...
     * （機能一覧 3.2）。**一般の一覧（参加者向け。`GET /workspaces/{id}/channels`）の API は、
     * この条件に加えて `AND c."archivedAt" IS NULL` が要る。** この形をそのまま一覧に写すと、
     * アーカイブ済みが一覧に出る。**自分が参加しているアーカイブ済みの一覧
     * （`GET /workspaces/{id}/archived-channels`）は逆に `IS NOT NULL` で絞る**
     * （機能一覧 3.2「アーカイブ済みのチャンネルを、参加者がどう読むか」）——
     * この一般の一覧の注意はそちらには当たらない。
     *
    ...
    it('パブリックチャンネルでも、参加していないメンバーは参加者一覧を取得できない', async () => {
      // **この条件は `visibility` を見ない**（機能一覧 3.1 の但し書き。提案・承認済・2026-09-13・依頼側）。
      // 開ける側に変えるなら、提案と承認の手順が要る。
      //
     ...
     *
     * **決定表の「返すもの」は `id` / 名前 / 種別 / 参加者数 / アーカイブ済みか と参加者一覧の6つだが、
     * この問い合わせが列として返すのは `id` / 名前 / 種別 / 参加者数の4つである。** アーカイブ済みかは
     * `archivedAt` から導く（この問い合わせは `archivedAt` で**絞らない**ことを固定するためのものであり、列には出していない）。
     * 参加者一覧は別の経路が返す
     * （`channelMemberList`。`channelMemberViewers` は**同じ経路の可否判定**であって、
     * 参加者一覧を返すものではない）。**「同じである」と書かない**——
     * この docblock を読んで決定表を開かない実装者に、管理の範囲が4項目として届き、アーカイブ済みかが落ちる。
     ...
     * 「一般の一覧（参加者向け）の API は、この条件に加えて `AND c."archivedAt" IS NULL`
     * が要る」と書いてあるが、
  ...
  describe('退会済みアカウントの照合', () => {
     ...
     * 参加者一覧を取得できる。**見せてよいのは管理用の範囲（`id` / 名前 / 種別 / 参加者数 / アーカイブ済みか と参加者一覧）だけである**（機能一覧 3.1 の決定表は、取得できる側を「だけ」で閉じている）。**メッセージ・添付ファイル・未読数・在席は、その外にある**——見せないものの側を数で閉じると、足した値（在席など）が見せてよい側に読める。
     *
     * **この形をそのまま写さないこと。** 値はプレースホルダとして渡す
     * （REVIEW.md 3 / CWE-89）。
     *
    ...
    it('参加していないオーナーは、参加者一覧を取得できる', async () => {
      ...
      // 見せてよいのは管理用の範囲（id / 名前 / 種別 / 参加者数 / アーカイブ済みか と参加者一覧）だけである（機能一覧 3.1 の決定表）。
      const output = await expectSqlToSucceed(
        channelMemberList({
          viewerId: '00000000-0000-7000-8000-000000000001',
          channelId: '00000000-0000-7000-8000-0000000000c2',
