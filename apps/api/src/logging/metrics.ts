 ...
 * - **現在値は接続・切断のときにだけ出す**（定期には出さない。タイマーを持たない）。**代償: 接続も切断も無い間は1点も出ず、
 *   このメトリクスだけでは「値が続いている」と「タスクが止まった」を区別できない**（全断は要件定義書 4.2 のとおり 5xx 率で捕まえるが、一部のタスクの停止はこのメトリクスでも 5xx 率でも捕まえない。接続数はアラートにしない）
