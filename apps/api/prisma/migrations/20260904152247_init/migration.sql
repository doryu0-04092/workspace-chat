...
-- Prisma のスキーマ言語に無い。
-- そのため **schema.prisma を読んでもこの4つは見えない。**
-- `prisma migrate dev` で次のマイグレーションを作るときも自動では復元されない。
--
-- 4つとも apps/api/src/prisma-schema.test.ts が実際の PostgreSQL に対して
-- 検証している。名前を変えるならテストも併せて直すこと。
-- ============================================================================

-- ユーザーID の一意性は大文字小文字を区別しない（機能一覧 1.1。2026-09-05 決定）。
--
-- **ユーザーID はメンションの `@` に続く識別子である**（機能一覧 1.1 / 9.1）。
-- 区別すると `@owner` と `@Owner` が別人を指し、**表示上ほぼ見分けがつかない。**
-- 同じ人を指しているつもりの言及が別人に届く状態は、なりすましに近い。
--
-- **F-36 の「ユーザーID を再利用させない」も、区別したままでは狙った範囲に届かない。**
-- `owner` が退会したあと `Owner` を登録できてしまい、過去のメンションが
-- 別人を指すのを防げない（機能一覧 1.5）。
--
-- **アプリ側の小文字化だけに頼らない。** 登録・招待・復旧など経路は複数あり、
-- **1箇所でも書き漏らせば、そのまま綴り違いの二重登録が通る。**
-- 認可と同一性の根拠は DB に置く（CLAUDE.md 2）。
--
-- 既定の照合順序で作られる `User_userId_key`（schema.prisma の `@unique`）は、
-- **この規則を止められない。** それでも残すのは、
-- **`schema.prisma` だけを読む人に「userId は一意である」が見えなくなるため**である。
-- 一意性の根拠はこの索引であり、`User_userId_key` は表示のために残している。
-- 代償: **一意性に何も足さない索引が1つ残り、書き込みのたびに更新される。**
--
-- 代償: **綴りだけが違うユーザーID は同時に存在できない**（使える ID の空間が狭まる）。
-- 列は登録時の綴りをそのまま保持するため、**照合する側も lower() で引くこと。**
-- 素の等値比較で書くと、登録時と違う綴りでは引けず、
-- ログインが「パスワードが違う」と区別できない失敗になる（機能一覧 1.2）。
CREATE UNIQUE INDEX "User_userId_lower_key" ON "User" (lower("userId"));
...
-- 0人になりうる経路は2つある。**どちらも DB は止めない。アプリ側が守る規約である。**
--
--   1. 作成時 … ワークスペースの作成時にオーナーの Membership を
--      同一トランザクションで作ること
--   2. 退会時 … 退会は「同一トランザクションで Membership を消す」と決めた
--      （機能一覧 1.5）。**オーナーの退会を拒否するのはアプリ側だけである**
--      （機能一覧 1.5「オーナーの場合は削除できない」）
--
-- **オーナー権限の委譲もワークスペースの削除も対象外である**（要件定義書 3.4）。
-- そのため0人になったワークスペースは**二度と管理できなくなる。**
-- 招待もチャンネルの作成もアーカイブもできず、復旧する手段が無い。
