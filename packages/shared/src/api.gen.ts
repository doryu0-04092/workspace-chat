...
export interface paths {
    ...
    "/workspaces/{id}/channels/{channelId}/messages/{messageId}": {
        ...
        /**
         * メッセージの編集（F-13）
         * @description 自分のメッセージだけを編集できる（作者でなければ 403 not_message_author。オーナーでも同じ。機能一覧 4.2）。 判定の順: 所属していなければ 404 → 参加していなければパブリックは 403 not_a_channel_member・プライベートは 404 → そのチャンネルに無い・削除済みのメッセージは 404 → 作者でなければ 403 not_message_author → アーカイブ済みのチャンネルは 409 channel_archived。 本文の形は投稿と同じ（1〜4000 文字・空白だけは不可）。編集したメッセージを message:updated としてチャンネルの部屋へ配る（機能一覧 5.2）。 チャンネルと DM の投稿・返信・編集・削除で1つの枠を分け合い、利用者単位で1分に60回まで（機能一覧 4.1・4.2・6・8）
         */
    ...
    "/workspaces/{id}/dms/{dmId}/messages": {
        ...
        /**
         * DM へのメッセージの投稿（F-19）
         * @description 当事者だけが投稿できる（所属していない・当事者でない・無い DM は 404）。 相手がいまそのワークスペースの退会していないメンバーでなければ 409 dm_counterpart_unavailable（機能一覧 8「相手は同一ワークスペースのメンバーに限る」）。 本文は 1〜4000 文字・空白だけは不可（チャンネルと違い、添付があっても本文を空にはできない。機能一覧 4.1・8）。 作ったメッセージを message:new として、当事者2人のうち、いまそのワークスペースの退会していないメンバーである利用者の部屋へ1回で配る（機能一覧 5.2 の DM の箇条）。 チャンネルと DM の投稿・返信・編集・削除で1つの枠を分け合い、利用者単位で1分に60回まで（機能一覧 4.1・4.2・6・8）
         */
    ...
    "/workspaces/{id}/dms/{dmId}/messages/{messageId}": {
         ...
         * @description 論理削除（機能一覧 4.2）。自分のメッセージだけを削除できる。判定の順は編集と同じ。本文は DB に残るが、 以後の応答と配信では返さない（body: null・deleted: true）。message:deleted（本文を載せない）を投稿と同じ宛先へ配る。 チャンネルと DM の投稿・返信・編集・削除で1つの枠を分け合い、利用者単位で1分に60回まで（機能一覧 4.1・4.2・6・8）
         */
         ...
         * @description 自分のメッセージだけを編集できる。判定の順: 所属していなければ 404 → 当事者でない・無い DM は 404 → その DM に無い・削除済みのメッセージは 404 → 作者でなければ 403 not_message_author。 相手がいまメンバーでなくても、自分のメッセージは編集できる。本文の形は投稿と同じ。 編集したメッセージを message:updated として、投稿と同じ宛先へ配る（機能一覧 5.2 の DM の箇条）。 チャンネルと DM の投稿・返信・編集・削除で1つの枠を分け合い、利用者単位で1分に60回まで（機能一覧 4.1・4.2・6・8）
         */
...
export interface components {
    schemas: {
        ...
        /** @description 一般の一覧（参加者向け）のチャンネル。作成（201）と、自分が参加しているアーカイブ済みの一覧 （GET /workspaces/{id}/archived-channels）の応答にも使う */
        Channel: {
            /** Format: uuid */
            id: string;
            name: string;
