...
# 通していない。**検査1 を足す前は**、除外の中の追跡対象ファイルをリンク先にすると、
# 本物のリポジトリでは通るのに、複製先には存在しないためテストの前提チェックだけが落ちた。
# これはディレクトリ側だけの話ではない。.env のような名前も doc_find の対象外であり、
# $work に複製されないため、同じずれが起きた。
# 原因の切り分けに時間がかかるため、check-docs.sh の検査1がこの制約を直接見るようにした。
# **いまはどちらも落ちる。** 検査1 は本物のリポジトリの実行でも走り、
# 除外に当たるリンク先を「リンク先にできない」で NG にする。
#
...
# check-docs.sh の検査1 が「リンク先にできない」で NG を出す
# （複製先にも入らないため、検査の検査は「存在しない」でも落ちる）。
# **どちらも落ちる。** ここから項目を外すときは、リンク元も併せて直す。
...
# パスの親を返す。親が無ければ `.`。
#
# **dirname を使わない。先頭が - のパスを option と解釈して失敗する**
# （実測: `dirname "-x.md"` は `unknown option -- x` で終了コード 1、標準出力は空）。
# `doc_find … | sed 's|^\./||'` を通ったパスは、リポジトリ直下のファイルでは
# **先頭の `./` が落ちて素の名前になる。** そこに `-x.md` が置かれるとこの形になる。
#
# 帰結（塞ぐ前に実際に起きること）:
#   - check-docs.sh の検査1 が `/docs/requirements.md` という**絶対パス**を見に行き、
#     そのファイルの相対リンクが全件「存在しない」で NG になる。
#     文面は「リンク切れ」と読めるため、**受け取った側は壊れていないリンクの方を疑う**
#
# **複製ループ（check-docs.test.sh）は、これに含めない。** dirname に戻しても
# `mkdir -p "$work/"` は既存ディレクトリに対して成功し、`cp "$repo/$p" "$work/$p"` も成功する。
# **パスは崩れない**（複製先を doc_parent の結果から組み立てていないため）。
# あちらに doc_parent を使っているのは、**規則を適用先の全体に当てた結果**であって、
# 「そこが壊れるから」ではない。
#
# **PR #37 が basename について同じ穴を `${p##*/}` で塞いでいる。これはその対である**（#47）。
# 塞いだのが片側だけになっていた。
doc_parent() { # $1=パス。その親を返す（親が無ければ .）
  local d=${1%/*}
  [ "$d" = "$1" ] && d=.
  printf '%s\n' "$d"
}
...
doc_excluded() { # $1=リンク先のパス。除外なら理由を出力して 0 を返す
  ...
  # そこは `$(doc_parent "$f")/$l` を渡す。doc_parent は最低でも `.` を返すため、
...
#
# ============================================================================
# なぜ「件数」を機械で照合するのか（REVIEW.md 4章から移した説明。2026-09-07）
# ============================================================================
#
# **コメントと文書に書かれた「位置に依存した参照」は、編集で黙ってずれる。**
# 行番号・相対の位置（「前の項目」「後述」「上の3番目」）・手書きの件数の3つがそれである。
# **いずれも、ずれても動作は壊れない。** そのため誰も気づかない。
#
# 実測。PR #29 の最後の巡回で出た指摘4件のうち3件がこれで、3件とも
# **直前の巡回で別の指摘を直したことによって生じていた**（「その前提として」の指す先が
# 挿入で変わった／「（後述）」の参照先が別ファイルへ移った／29行下の分岐を直した結果、
# 上の説明と食い違った）。位置に依存した参照がある限り、1件直すたびに別の参照がずれる。
#
# **このうち「件数」は、固定書式の宣言に書かれたものだけ機械で照合できる。**
# その分だけをレビューの観点から外し、
# ここと check-docs.sh が引き受ける（REVIEW.md 0章「誰が見るか」の3分割）。
# **固定書式の宣言に一致しない、本文中の手書きの件数は誰も見ない。**（例外は REVIEW.md 9.1 の E の台帳の行1つだけ）
# 下の decls の正規表現に一致するものだけが照合の対象である。ここを
# 「件数は CI が見る」と広く読むと、照合されない件数を書いて通すことになる。
# 行番号と相対の位置は照合できないため、**誰も見ない**（REVIEW.md 7章）。
#
# 照合している宣言は check-docs.sh の検査3・5 と check-docs.test.sh の「N通り」にある。
# **どれも消してはならない。** 照合は「読み取れない」も NG にするため、
# 規則違反と読んで消すか言い回しを変えると docs が落ちる。
# 照合が依存している見出しと表の書式も同じ扱いである。
#
# **書くなら、機械が照合できる形にする。照合できないなら書かない。**
# **「機械が照合できる形」には2つある。** (1) ここと check-docs.sh が照合する固定書式の宣言。
# (2) **実行時に実数から導く**（例: lint-scope.test.sh の begin_check は検査の数を数えて総括に出す）。
# (2) は照合する道具を持たないが、手で書いた数ではないためずれない。**手で書いた数だけを禁じている。**
# 宣言に含まれる数をすべて返す。check-docs.sh の宣言の読み取りと、check-docs.test.sh の
