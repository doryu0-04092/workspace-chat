...
# その分だけをレビューの観点から外し、
# CI が引き受ける（照合している宣言の置き場は REVIEW.md 7章。0章「誰が見るか」の3分割）。
...
#
# 照合している宣言（置き場は REVIEW.md 7章）は、**どれも消してはならない。** 照合は「読み取れない」も NG にするため、
# 規則違反と読んで消すか言い回しを変えると docs が落ちる。
# 照合が依存している見出しと表の書式も同じ扱いである。
#
# **書くなら、機械が照合できる形にする。照合できないなら書かない。**
# **「機械が照合できる形」には2つある。** (1) 固定書式の宣言（置き場は REVIEW.md 7章）。
