...
for f in "${docs[@]}"; do
  ...
  # **`./` を前置するのは、doc_find の出力が素の名前になりうるためである**（#47。doc_parent の注記）。
  # **awk はこれを option と解釈しない**（gawk 5.4.0 で実測。プログラム文字列より後ろは operand）。
  # **1つの書き方で揃えるために前置している。** 落ちるのは下の grep と doc_parent のほうである。
...
# docs/tech-stack.md は現に「8種類の WebSocket イベント」「8種類のイベント定義」を語る文書で、
...
# そのとき 5.1 は「8種類」とだけ書いてあって、どこにも一覧が無い節になる。
...
if [ "$kinds" -eq 0 ]; then
  ...
  #
  # requirements.md の宣言は「（4.1 の8種類）は、フロントエンドと…」である。
