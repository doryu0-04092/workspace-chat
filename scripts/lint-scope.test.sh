...
#
# **この2つ目を、ファイル名（lint-scope.test.sh）も冒頭の宣言も長らく含んでいなかった。** ファイルを頭から
# 読んだ人が「react-hooks のルール」の節に来て初めて役割の広がりを知る状態だったため、
...
#
# ここまでの走査範囲の確認に加え、react-hooks のルールの配線も機械で確かめる
# （冒頭の宣言のとおり、この検査が見るのは走査範囲だけではない）。
#
...
#     **この判定だけがその経路を見ている**（設定から行ごと消す壊し方はこの判定自身が拾う。
#     件数の判定が拾うのは、error のまま書き方だけを配列形式に変えた場合である）
...
# **パイプを使わない。** この制約は判定そのもの（一致したかどうかで
# 合否を反転させる箇所）に掛かる。**判定を終えたあとの診断出力**
# （check_tool が NG の直前に出す抜粋）は結果を左右しないため対象外——
# 対象を「一致・不一致の反転がありうる箇所」に絞らないと、範囲が
# ファイル全体に広がって読み取れなくなる。
#
# `printf '%s' "$1" | grep -qF "$2"` にすると、
...
# 検査の見出しを出し、**同時に数える。**
#
# 総括が出す件数を手で書かない（規則と理由は scripts/doc-scope.sh の decls の直上にある。**手で書いた数を禁じており、
# 実行時に実数から導く形は認めている**）。このスクリプトの件数を照合している道具は無く、check-docs.sh も
...
# 「ESLint」の節の直上に書いた理由は、ここでも npm run lint を経由することで
# 保っている）。
if [ "$ESLINT_CODE" -ne 0 ] && [ "$ESLINT_CODE" -ne 1 ]; then
  ...
  if [ "$hooks_broken_before" -ne 1 ] || [ "$hooks_broken_after" -ne 1 ]; then
    ...
    # 上の「ESLint」の節の直上が塞ごうとした穴が、ここには開いたままである。
