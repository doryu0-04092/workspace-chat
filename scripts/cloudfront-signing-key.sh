...
#
#   bash scripts/cloudfront-signing-key.sh [鍵の名前（既定: signing-1）]
#
# 前提: openssl と AWS CLI。AWS CLI の資格情報が本番のアカウントを向いていること。
#
# 流れ:
#   1. 秘密鍵（RSA 2048）を一時ディレクトリに作り、公開鍵を infra/production/keys/<名前>.pem に書く
...
#
# 踏むと壊れる:
# - パラメータの名前は infra/production/compute.tf の cloudfront_private_key_parameter_name と、既定の鍵の名前は
...
public_key="infra/production/keys/${name}.pem"
